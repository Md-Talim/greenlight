...
// Valid returns true if the errors map doesn't contain any errors
func (v *Validator) Valid() bool {
	return len(v.Errors) == 0
}

// AddError adds an error message to the map (so long as no entry exists for
// the given key)
...
// Generic function returns true if returns true if a specific value
